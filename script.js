...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Функция, которая делает текст аккуратным, розовым и без свечения
    const applyBeautifulStyle = (el, size) => {
        if (!el) return;
        el.style.transition = "opacity 0.4s ease-in-out, transform 0.4s ease-in-out";
        el.style.color = "#db2777"; // Чистый розовый цвет
        el.style.textShadow = "none"; // Полностью убрали свечение
