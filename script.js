...
window.turnPage = function() {
    ...
    if (bookStep === 0) {
        // Шаг 1: Центрируем книгу и открываем обложку
        if (book) book.classList.add('opened');
        if (p1) p1.classList.add('flipped');
        bookStep = 1;
    } else if (bookStep === 1) {
        // Шаг 2: Перелистываем вторую страницу
        if (p2) p2.classList.add('flipped');
        bookStep = 2;
    } else if (bookStep === 2) {
        // Шаг 3: Переходим к мозаичному сердцу
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Текст меняется каждые 2 секунды
    setTimeout(() => {
    ...
    // Завершение матрицы и открытие фотоальбома через 6.5 секунд
    setTimeout(() => {
