// --- ЛОГИКА МУЗЫКИ (AUTOPLAY + СТРАХОВКА) ---
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (music && musicBtn) {
        ...
        if (playPromise !== undefined) {
            playPromise.then(() => {
                // Если браузер разрешил автоплей — супер!
                musicBtn.classList.remove('muted');
                musicBtn.classList.add('playing');
            }).catch(error => {
                // Если заблокировал — не страшно, ждем первого клика
                console.log("Браузер заблокировал автоплей. Ждем первого клика.");
            });
        }

        // Функция ручного переключения музыки по кнопке
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Переход к книге на фоне 1.jpeg
    setTimeout(() => {
