...
const TEXTS = {
    ...
    // ОБЪЕДИНЕННЫЙ ТЕКСТ ДЛЯ РАЗВОРОТА:
    spread: "Каждый день рядом с тобой приносит невероятную радость и тепло. 💕\nТы заставляешь этот мир сиять ярче, и я благодарен за каждый миг! 🌸",
...
function typeWriter(elementId, text, speed = 40) {
    ...
    element.typewriterInterval = setInterval(() => {
        if (i < text.length) {
            // Если в тексте встречается символ переноса строки \n, заменяем его на <br>
            if (text.charAt(i) === '\n') {
